...
const adModeRW = 3	'adModeReadWrite
const adTypeB = 1	'adTypeBinary
const adTypeT = 2  	'adTypeText
const adSaveCreateOverW = 2	'adSaveCreateOverWrite
...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			streamFile.Type = adTypeB
			streamFile.Open
			StreamRequest.Position=fileItem.Start
			StreamRequest.CopyTo streamFile, fileItem.Length
			streamFile.SaveToFile filePath, adSaveCreateOverW
			streamFile.close
	...
	public sub SaveOne(path, num, byref outFileName, byref outLocalFileName)
		...
		if UploadedFiles.Count > 0 then
			...
			StreamRequest.Position = fileItem.Start
			StreamRequest.CopyTo streamFile, fileItem.Length
			streamFile.SaveToFile path & outLocalFileName, adSaveCreateOverW
			streamFile.close
			Set streamFile = Nothing
	...
	public Function SaveOneKey(path, strKey)
		Dim streamFile, fileItems, fileItem, fs, outFileName, outLocalFileName
		outLocalFileName = ""
		...
		if UploadedFiles.Count > 0 Then
			'fileItems = UploadedFiles.Items
			If UploadedFiles.Exists(strKey) Then 

				set fileItem = UploadedFiles.item(strKey)
		...
		SaveOneKey = outLocalFileName
	end Function
	
	public Function SaveOneFile(path)
		Dim streamFile, fileItems, fileItem, fs, outFileName, outLocalFileName
	...
	Private Function ConvertUtf8BytesToString(start, length)	
	    ...
	    objStream.Mode = adModeRW
	    objStream.Type = adTypeB
	    objStream.Open
