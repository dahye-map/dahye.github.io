...
$(document).ready(function(){
    ...
    var nav = {
        ...
        addEvent:function(){
            ...
            $('#navWrap .nav > ul > li > a').on('click',function(e){
                var _siblings = $(this).siblings('ul');
                if(_siblings.length){
                    e.preventDefault();
                    $('#navWrap .nav > ul > li').removeClass('active');
                    $(this).parent().addClass('active');
                    $('#navWrap .nav > ul > li > ul').stop().slideUp();
                    _siblings.stop().slideToggle();
                }
            })
        },
        activeFunc:function(_oneNum, _twoNum){
            var _siblings;
            if(_oneNum != -1){
                _siblings = $('#navWrap .nav > ul > li').eq(_oneNum).find('ul');
    ...
    $('.common-check.all-chk input').click(function(){
        ...
        if($(this).is(":checked")){
            ...
            $('.table-wrap tr').addClass('bg-color');
        }else{
            $("[name='" + _thisName + "']").prop("checked", false);
            $('.table-wrap tr').removeClass('bg-color');
        }
        //disabled 시 체크 안됨
        if(_disableCheck > 0){
            $('input:disabled').parents('tr').removeClass('bg-color');
