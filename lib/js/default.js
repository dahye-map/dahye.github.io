...
function addBlock() {
    ...
    $('body, html, #containerWrap').css({'overflow': 'hidden', 'height' : 'winH'});
    $('.block').fadeIn(300);
    $('.block').on('click', function() {
        ...
        $('body, html, #containerWrap').css({'overflow': '','height': ''});
    });
}
...
function customSelect() {
    if ($('.select').length) {
        ...
        $('.select li a').on('click',function(e){
            e.preventDefault();
            _selectThis = $(this);
            _selectText = _selectThis.text();

            selectAction(_selectText);
            if (_selectThis.parent().hasClass('join_cate') || _selectThis.parent().hasClass('join_brand')) {
                if (_selectThis.parent().hasClass('join_cate')) {
                    _selectThis = $('.join_brand').eq(0).find('a');

                }else if(_selectThis.parent().hasClass('join_brand')){
                    _selectThis = $('.join_cate').eq(0).find('a');
                }
                _selectText = _selectThis.text();
            }
            selectAction(_selectText);
...
var _selectThis, _selectText;
function selectAction(_selectText){
    _selectThis.parent().parent().find('li').removeClass('select');
    _selectThis.parent().addClass('select');
    _selectThis.parent().parent().slideUp('fast').siblings('p').text(_selectText);
    _selectThis.parent().parent().parent().removeClass('active');
}
